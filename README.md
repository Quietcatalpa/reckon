...
- **🧹 清理**：扫描 C/D 盘，给每个候选文件一个「删除把握」和理由，分成
...
**清理**：每一项都有删除把握和理由，拿不准的放进「需要你看」

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/images/clean-dark.png">
  <img alt="清理页面：按删除把握分成建议删除、需要你看、重复文件、缓存目录、建议保留" src="docs/images/clean-light.png" width="860">
</picture>
</div>
...
<picture>
  ...
  <img alt="整理页面：每个文件显示建议的目标文件夹、理由和把握" src="docs/images/organize-light.png" width="860">
</picture>
</div>
...
flowchart LR
    ...
    R1 & R2 & R3 & L --> F[删除把握 + 理由] --> G[你勾选] --> H[移到回收站]
```
...
**Laya 判断得准吗？**
在「删除还是保留」这个问题上，我们用 12 个真实文件测试，答对 9 个，另外 3 个把握在 50% 左右，会落进「需要你看」。
